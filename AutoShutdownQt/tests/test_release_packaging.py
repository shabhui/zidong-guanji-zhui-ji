...
class ReleasePackagingTest(unittest.TestCase):
    def _valid_manifest(self, archive_name="AutoShutdownQt-2.0.zip", bundle="AutoShutdownQt-2.0"):
        return {
            "app": "AutoShutdownQt",
            "version": "2.0",
            "bundle": bundle,
    ...
    def _write_valid_archive(self, archive_path, manifest=None):
        ...
        with zipfile.ZipFile(archive_path, "w") as archive:
            ...
            archive.writestr("AutoShutdownQt-2.0/release-manifest.json", json.dumps(manifest))
    ...
    def test_release_archive_validation_fails_when_qml_is_missing(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with zipfile.ZipFile(archive_path, "w") as archive:
                archive.writestr("AutoShutdownQt-2.0/AutoShutdownQt.exe", "exe")
                archive.writestr("AutoShutdownQt-2.0/release-manifest.json", json.dumps(self._valid_manifest(archive_path.name)))
    ...
    def test_release_archive_validation_fails_when_main_qml_is_missing(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with zipfile.ZipFile(archive_path, "w") as archive:
                archive.writestr("AutoShutdownQt-2.0/AutoShutdownQt.exe", "exe")
                archive.writestr("AutoShutdownQt-2.0/_internal/qml/Theme.qml", "qml")
                archive.writestr("AutoShutdownQt-2.0/release-manifest.json", json.dumps(self._valid_manifest(archive_path.name)))
    ...
    def test_release_manifest_records_version_and_safety_notes(self):
        with tempfile.TemporaryDirectory() as tmp:
            bundle = Path(tmp) / "AutoShutdownQt-2.0"
    ...
    def test_release_notes_document_portable_dry_run_and_unsigned_status(self):
        notes = (ROOT / "RELEASE_NOTES_v2.0.md").read_text(encoding="utf-8")
        self.assertIn("Dry-run", notes)
        self.assertIn("便携版", notes)
        self.assertIn("未做代码签名", notes)
        self.assertIn("dist/AutoShutdownQt-2.0.zip", notes)
